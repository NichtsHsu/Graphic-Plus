...
extern "C" void freeBoxBlurKernel()
{
	if (oriImg != NULL)
		cudaFree(oriImg);
	if (blurImg != NULL)
...
extern "C" void reMallocBoxBlur(int width, int height)
{
	if (oriImg != NULL)
		cudaFree(oriImg);
	if (blurImg != NULL)
...
extern "C" void addBoxBlurKernel(unsigned *imageData, int *args)
{
	cudaSetDevice(0);
	if (oriImg == NULL)
		reMallocBoxBlur(args[0], args[1]);
	...
	{
		BoxBlur <<< grid, block >>>(blurImg, oriImg, args[1], args[0]);
